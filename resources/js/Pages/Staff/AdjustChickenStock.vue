            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <form v-if="activeForm === 'add'" @submit.prevent="submitAddition" class="mt-6 space-y-4">
                    <div>
                        ...
                        <input v-model="addForm.quantity" type="number" id="add_quantity" class="mt-1 block w-full rounded-md border-gray-300" placeholder="e.g., 100">
                         <div v-if="addForm.errors.quantity" class="text-sm text-red-600 mt-1">{{ addForm.errors.quantity }}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea v-model="addForm.notes" id="add_notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300" placeholder="e.g., From supplier X"></textarea>
                    </div>
                ...
                <form v-if="activeForm === 'remove'" @submit.prevent="submitRemoval" class="mt-6 space-y-4">
                     <div>
                        ...
                        <input v-model="removeForm.quantity" type="number" id="remove_quantity" class="mt-1 block w-full rounded-md border-gray-300" placeholder="e.g., 20">
                         <div v-if="removeForm.errors.quantity" class="text-sm text-red-600 mt-1">{{ removeForm.errors.quantity }}</div>
                    </div>
                     <div>
                     ...
                     <div>
                        ...
                        <textarea v-model="removeForm.notes" id="remove_notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300" placeholder="e.g., Transferred to Farm Y"></textarea>
                    </div>
                        ...
                        <tbody>
                            <tr v-for="log in recentAdjustments" :key="log.id" class="border-b">
                                ...
                                <td class="py-2 text-sm font-bold text-right" :class="{'text-green-600': log.adjustment_type === 'addition', 'text-red-600': log.adjustment_type === 'removal'}">
                                 ...
                                 <td class="py-2 text-sm text-gray-600 text-right">{{ log.reason }}</td>
                            </tr>
                        </tbody>
                     </table>
                 </div>
